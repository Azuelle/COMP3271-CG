...
void FrameBufferSizeCallback(GLFWwindow *window, int width, int height) {
    SCR_WIDTH = width;
    SCR_HEIGHT = height;
    glViewport(0, 0, SCR_WIDTH,
               SCR_HEIGHT);  // Set the viewport to cover the new window
    aspect = (float)SCR_WIDTH /
             (float)SCR_HEIGHT;  // Set the aspect ratio of the clipping area to
                                 // match the viewport
}
}  // namespace

void MainFrame::LeftMouseMove(float start_x, float start_y, float curr_x,
                              float curr_y) {
    if (modeling_state_ == OBJ_ROTATION) {
        // ------------------------ Object Rotation --------------------------
        // TODO 2: Add your code here.
        // Find the correct 4x4 transform matrix "transform_mat" to rotate the
        // object about its center.
        ...
        glm::vec3 rot_axis =
            glm::normalize(Screen2World(A + start) - Screen2World(start));

        // 4. find the rotation angle k * ||A||, assign a proper value to k
        float rot_angle = glm::length(A) * 0.003f;

        // 5. find the rotation matrix
        glm::mat4x4 rot_mat =
            glm::rotate(glm::mat4x4(1.f), rot_angle, rot_axis);
        ...
        // Find the correct 4x4 transform matrix "trans_mat" to translate the
        // object along the view plane.
        ...
        // Find the correct 4x4 transform matrix "trans_mat" to translate the
        // face vertices along the face normal.
        ...
        // 2. calculate the face normal vector in the world space using
        // "mesh_.faces_" and "mesh_.vertices_"
        // possible useful functions: glm::cross, glm::normalize
        glm::vec3 v_a = mesh_.vertices_[mesh_.faces_[face_index][0]];
        glm::vec3 v_b = mesh_.vertices_[mesh_.faces_[face_index][1]];
        glm::vec3 v_c = mesh_.vertices_[mesh_.faces_[face_index][2]];
        glm::vec3 face_normal =
            glm::normalize(glm::cross(v_a - v_b, v_b - v_c));

        // 3. find the point on ray (intersected_point, face_normal) which is
        // the closest to ray (o, v_cur)
        glm::vec3 pdest =
            glm::normalize(v_cur) * glm::dot(v_cur, (intersected_point - o));
        glm::vec3 vdest =
            face_normal * glm::dot(face_normal, pdest + o - intersected_point);

        // 4. find the translation matrix to move the face vertices along the
        // face normal direction to the new point
        transform_mat = glm::translate(glm::mat4x4(1.f), vdest);
        // glm::translate(glm::mat4x4(1.f), face_normal);
...
void MainFrame::MainLoop() {
    ...
    GLFWwindow *window =
        glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "3DModeling", NULL, NULL);
    ...
    const float r = 5.f;
    camera_.LookAt(
        r * glm::vec3(std::cos(alpha) * std::cos(beta),
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glClear(GL_COLOR_BUFFER_BIT |
                GL_DEPTH_BUFFER_BIT);  // Clear the display
...
void MainFrame::ProcessInput(GLFWwindow *window) {
    ...
    if (current_l_mouse_state == GLFW_PRESS) {
        ...
        if (l_mouse_state_ == GLFW_PRESS &&
            (std::abs(xpos - last_cursor_x_) > 2.f ||
             std::abs(ypos - last_cursor_y_) > 2.f)) {
...
void MainFrame::LeftMouseClick(float x, float y) {
    if (modeling_state_ == OBJ_SUBDIVIDE) {
        ...
        glm::vec3 cam_pos =
            camera_.view_mat_inv_ * glm::vec4(0.f, 0.f, 0.f, 1.f);
...
void MainFrame::LeftMouseRelease() { mesh_.CommitTransform(); }

void MainFrame::RightMouseClick(float x, float y) { return; }

void MainFrame::RightMouseMove(float start_x, float start_y, float curr_x,
                               float curr_y) {
    ...
    glm::vec3 rot_axis =
        glm::normalize(Screen2World(A + s_start) - Screen2World(s_start));
    glm::mat4x4 rot_mat =
        glm::rotate(glm::mat4x4(1.f), 0.007f * glm::length(A), rot_axis);
    camera_.ApplyTransform(rot_mat);
}

void MainFrame::RightMouseRelease() { return; }
...
std::tuple<glm::vec3, glm::vec3> MainFrame::Screen2WorldRay(float scr_x,
                                                            float scr_y) {
    float x = scr_x / SCR_WIDTH * 2.f - 1.f;
    float y = scr_y / SCR_HEIGHT * 2.f - 1.f;
    float focal = std::tan(camera_.fov_ * .5f / 180.f * glm::pi<float>());
