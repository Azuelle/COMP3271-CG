...
Color Shade(const Hittable &hittable_collection, const HitRecord &hit_record,
            int trace_depth);

void tracePixel(int x, int y, int width, int height, int thread_spp,
                const Camera &camera, const Scene &scene, float *image_buffer) {
...
void thread_func(int thread_i, int n_thread, int spp, int width, int height,
    ...
    for (int x = thread_i; x < width; x += n_thread) {
        for (int y = 0; y < height; ++y) {
            tracePixel(x, y, width, height, thread_spp, camera, scene,
                       image_buffer);
        }
    }
    ...
    std::cout << "Thread " << thread_i + 1 << " finished in " << elapsed.count()
              << " seconds." << std::endl;
}
...
int main() {
    ...
    for (int thread_i = 0; thread_i < NUM_THREAD; ++thread_i) {
        thread_pool.emplace_back(thread_func, thread_i, NUM_THREAD, spp, width,
                                 height, std::ref(camera), std::ref(scene),
                                 image_buffer);
    }
    ...
    for (int i = 0; i < width * height; ++i) {
        for (int j = 0; j < 3; ++j) {
            image[4 * i + j] = static_cast<uint8_t>(
                glm::min(image_buffer[3 * i + j], 1.f - 1e-5f) * 256.f);
        }
        image[4 * i + 3] = 255;
    }
...
Vec SampleHemisphere(const Vec &normal) {
    ...
    // Hint: sample a random direction on the hemisphere with normal as the
    // z-axis Hint: Use Rodrigues' rotation formula to rotate the z-axis to the
    // normal direction

    float theta = 2.f * M_PI * get_random_float();
    float phi = .5f * M_PI * get_random_float();
    ray = Vec(sin(phi) * cos(theta), sin(phi) * sin(theta), cos(phi));
    // Rodrigues' rotation formula
    Vec k = glm::cross(Vec(0.0f, 0.0f, 1.f), normal);
    float cos_theta = glm::dot(Vec(0.0f, 0.0f, 1.f), normal);
    float sin_theta = sqrt(1 - cos_theta * cos_theta);

    if (glm::length(k) < 1e-6) {
        return (cos_theta > 0) ? ray : -ray;
    }
    k = glm::normalize(k);
    ray = ray * cos_theta + glm::cross(k, ray) * sin_theta +
          k * glm::dot(k, ray) * (1 - cos_theta);
...
Color Shade(const Hittable &hittable_collection, const HitRecord &hit_record,
    ...
    if (material.k_d > 0.f && trace_depth < kMaxTraceDepth) {
        ...
        Vec diffuseReflectance =
            TraceRay(Ray(o, wi), hittable_collection, trace_depth + 1);
        Vec BRDF = material.diffuse / M_PI;
        float pdf = 1.f / 2.f / M_PI;
        color += glm::dot(diffuseReflectance, BRDF) * cos_theta / pdf;
    }
    // 3. calculate specular
    if (material.k_s > 0.f && trace_depth < kMaxTraceDepth) {
        // Glossy specular reflection using Phong BRDF
        Vec reflection = glm::normalize(
            glm::reflect(hit_record.in_direction, hit_record.normal));
        ...
        Vec brdf = material.specular * ((material.sh + 2) / (2 * M_PI)) *
                   (float)pow(cosAlpha, material.sh);
        float pdf =
            ((material.sh + 1) / (2 * M_PI)) * pow(cosAlpha, material.sh);
        Vec reflectColor = TraceRay(Ray(hit_record.position, newDir),
                                    hittable_collection, trace_depth + 1);
